...
const getPetitionStatus = async (req, res) => {
    try {
        // Check if petition ID is provided
        ...
        // Determine if the request is from a user or an official
        if (req.user && Array.isArray(req.user.role) && req.user.role[0] === "user") {
            // For users, use the email field
            whereCondition.email = req.user.email;
        } else if (req.official) {
            // For officials, use the assigned_official_id field
...
const getRecentActivity = async (req, res) => {
    try {
        const petitions = await model.petiguard.petitionForm.findAll({
            where: { email: req.user.email },
            order: [['updatedAt', 'DESC']], // Order by latest update
            limit: 5, // Limit to the 5 most recent updates (adjust as needed)
            attributes: ['petitionId', 'status', 'updatedAt', 'petitionType', 'description'] // Include relevant details
