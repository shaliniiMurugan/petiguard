...
const Statistics = () => {
    ...
    const processApiData = (data) => {
        ...
        const statusStats = Object.keys(statuses)
            .filter(status => statuses[status].count > 0)
            .map(name => {
    ...
    const getBarChart = () => {
        ...
        const maxValue = Math.max(...statsData.monthlyStats.map(item => Math.max(item.petitions, item.resolved)));
                        ...
                        <div className="bars">
                            <div className="bar-container">
                                <div
                                    ...
                                    style={{ height: `${maxValue > 0 ? (month.petitions / maxValue) * 100 : 0}%` }}
                                >
                                    <span className="bar-value">{month.petitions}</span>
                                </div>
                                <div
                                    className="bar resolved-bar"
                                    style={{ height: `${maxValue > 0 ? (month.resolved / maxValue) * 100 : 0}%` }}
                                >
    ...
    return (
        <div className="statistics-container">
            ...
            <div className="statistics-grid">
                ...
                {/* {statsData.departmentStats.length > 0 && (
                                ...
                                <div className="bar-chart-horizontal">
                                    {statsData.departmentStats.map((dept, index) => (
                                        <div key={index} className="horizontal-bar-item">
                                            ...
                                            <div className="bar-container">
                                                <div
                                                    className="horizontal-bar department-bar"
                                                    style={{ width: `${dept.percentage}%` }}
                                                >
                                                    <span className="bar-value">{dept.count}</span>
                                                </div>
                                            </div>
                                            <div className="percentage">{dept.percentage}%</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
